...
# 現在のターミナルサイズを取得（detached セッションは最小サイズになるため）
TERM_WIDTH=$(tput cols 2>/dev/null || echo 200)
TERM_HEIGHT=$(tput lines 2>/dev/null || echo 50)

# セッション作成（1ペイン目）- ディスプレイサイズに合わせる
tmux new-session -d -s multiagent -n "agents" -x "$TERM_WIDTH" -y "$TERM_HEIGHT"
...
# ═══════════════════════════════════════════════════════════════════════════════
log_war "👑 将軍の本陣を構築中..."
tmux new-session -d -s shogun -x "$TERM_WIDTH" -y "$TERM_HEIGHT"
...
tmux select-pane -t "$SHOGUN_PANE" -P 'bg=#002b36'  # 将軍の Solarized Dark
...
NINJA_EOF
    ...
    # 家老に指示書を読み込ませる（最初のペイン）
    sleep 2
    log_info "  └─ 家老に指示書を伝達中..."
    tmux send-keys -t "${PANE_IDS[0]}" "instructions/karo.md を読んで役割を理解せよ。"
    sleep 0.5
